...
/*   Updated: 2022/04/29 15:50:11 by tmartial         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int		valInt = std::atoi(string.c_str());
	double	valFloat = std::atof(string.c_str());

    if (string.length() == 1 && isprint(string[0]) && string[0] != '0' && !valInt)
	{
		this->_charVal = string[0];
		this->_intVal = static_cast<int>(string[0]);
		this->_floatVal = static_cast<float>(string[0]);
		this->_doubleVal = static_cast<double>(string[0]);
	}
    else
    {
        this->_charVal = static_cast<int>(valInt);
        this->_intVal = valInt;
        this->_floatVal = static_cast<float>(valFloat);
        this->_doubleVal = static_cast<double>(valFloat);
	}
...
{
	if (this->_charVal < 32 && this->_string.length() == 1)
...
{
	if (this->_string != "0" && this->_intVal == 0)
		throw Scalar::Impossible();
	return this->_intVal;
}
